...
if (process.env.NODE_ENV === 'test') {
  // For tests, load test environment only
  if (fs.existsSync(envPath)) {
    require('dotenv').config({ path: envPath });
  }
} else {
  // For production/development, load root first, then backend
...
app.use(cors({
  origin: function (origin, callback) {
    ...
    // In development/test, allow localhost with specific ports only (3000-3010)
...
// Apply rate limiting to all API routes (skip in test mode)
if (process.env.NODE_ENV !== 'test') {
